...
export class ExpensesService {
  ...
  async toggleCategoryActive(id: number) {
    const category = await this.findOneCategory(id);
    return this.prisma.expenseCategory.update({ where: { id }, data: { active: !category.active } });
  }
  ...
  async findAll(options?: {
    ...
    if (options?.paymentMethod) where.paymentMethod = options.paymentMethod as any;
  ...
  async getStats(options?: { dateFrom?: string; dateTo?: string }) {
    ...
    const todayResult = await this.prisma.expense.aggregate({
      where: { ...where, createdAt: { gte: todayStart, lt: todayEnd } },
      _sum: { amount: true },
      _count: true,
    });
    ...
    const byCategoryWithNames = byCategory.map(item => {
      ...
      return {
        ...
        categoryNameAr: cat?.nameAr || cat?.name || 'بدون تصنيف',
    ...
    const recurringCount = await this.prisma.expense.count({ where: { ...where, isRecurring: true } });
  ...
  async getMonthlyTrend() {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const key = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
      result.push({ month: key, monthName: monthNames[date.getMonth()], total: monthlyData[key] || 0 });
    }

    return result;
  }

  async exportExpenses(options?: { categoryId?: number; dateFrom?: string; dateTo?: string }) {
    const where: Prisma.ExpenseWhereInput = {};
    if (options?.categoryId) where.categoryId = options.categoryId;
    if (options?.dateFrom || options?.dateTo) {
      where.expenseDate = {};
      if (options.dateFrom) (where.expenseDate as any).gte = new Date(options.dateFrom);
      if (options.dateTo) (where.expenseDate as any).lte = new Date(options.dateTo);
    }
