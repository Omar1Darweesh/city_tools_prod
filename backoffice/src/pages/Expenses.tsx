...
export default function Expenses() {
    ...
    useEffect(() => { fetchStats(); fetchExpenses(); }, [fetchStats, fetchExpenses]);

    // Quick period filter
    const applyPeriod = (id: string) => {
        ...
        if (id === 'today') {
            ...
            setFilters(f => ({ ...f, dateFrom: d.toISOString().split('T')[0], dateTo: now.toISOString().split('T')[0] }));
        } else if (id === 'month') {
            const first = new Date(now.getFullYear(), now.getMonth(), 1);
            setFilters(f => ({ ...f, dateFrom: first.toISOString().split('T')[0], dateTo: now.toISOString().split('T')[0] }));
        } else {
    ...
    const resetFilters = () => {
        ...
        setFilters({ search: '', categoryId: '', paymentMethod: '', dateFrom: d.dateFrom, dateTo: d.dateTo, sortBy: 'createdAt', sortOrder: 'desc' });
        setPage(1);
    };
    ...
    const chipStyle = (id: string): React.CSSProperties => ({
        background: selectedPeriod === id ? '#6366f1' : '#f1f5f9',
        color: selectedPeriod === id ? 'white' : '#475569',
        border: selectedPeriod === id ? '2px solid #6366f1' : '2px solid transparent',
    ...
    return (
        <div style={{ padding: '24px', direction: 'rtl', fontFamily: 'Cairo, sans-serif', background: '#f8fafc', minHeight: '100vh' }}>
            ...
            <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', flexWrap: 'wrap' }}>
                <StatCard icon={<DollarSign size={24} />} label="إجمالي الفترة المختارة"
                    value={stats ? fmt(stats.totalExpenses) + ' ج.م' : '...'} subValue={stats ? `${stats.totalCount} مصروف` : ''}
                    bg="#fee2e2" iconColor="#dc2626" valueColor="#dc2626" />
                <StatCard icon={<Calendar size={24} />} label="مصروفات اليوم"
                    value={stats ? fmt(stats.todayExpenses) + ' ج.م' : '...'} subValue={stats ? `${stats.todayCount} مصروف` : ''}
                    bg="#dcfce7" iconColor="#16a34a" />
                <StatCard icon={<BarChart3 size={24} />} label="مصروفات الشهر"
                    value={stats ? fmt(stats.monthExpenses) + ' ج.م' : '...'} subValue={stats ? `${stats.monthCount} مصروف` : ''}
                    bg="#dbeafe" iconColor="#3b82f6" />
                <StatCard icon={<Repeat size={24} />} label="مصروفات متكررة"
                    value={stats?.recurringCount || 0} bg="#fef3c7" iconColor="#d97706" />
            </div>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    {categories.length === 0 && (
                        <p style={{ color: '#94a3b8', fontSize: '14px' }}>لا توجد تصنيفات. أضف تصنيفاً أولاً.</p>
                    )}
                    {categories.map(cat => (
                        <div key={cat.id} style={{
                            ...
                            <div style={{ display: 'flex', gap: '4px' }}>
                                <button onClick={() => openCategoryModal(cat)} style={{
                                    background: '#f1f5f9', border: 'none', borderRadius: '6px', padding: '6px', cursor: 'pointer', color: '#64748b'
                                ...
                                {cat._count.expenses === 0 && (
                                    <button onClick={() => setDeleteModal({ type: 'category', id: cat.id })} style={{
                                        background: '#fee2e2', border: 'none', borderRadius: '6px', padding: '6px', cursor: 'pointer', color: '#dc2626'
            ...
            <div style={{
                ...
                <div style={{ marginBottom: '16px' }}>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        <button onClick={() => applyPeriod('today')} style={chipStyle('today')}>اليوم</button>
                        <button onClick={() => applyPeriod('7d')} style={chipStyle('7d')}>آخر 7 أيام</button>
                        <button onClick={() => applyPeriod('month')} style={chipStyle('month')}>هذا الشهر</button>
                        <button onClick={() => applyPeriod('all')} style={chipStyle('all')}>الكل</button>
                    </div>
                    {(filters.dateFrom || filters.dateTo) && (
                        <div style={{ marginTop: '6px', fontSize: '12px', color: '#6366f1', display: 'flex', alignItems: 'center', gap: '5px' }}>
                            <Calendar size={12} />
                            {filters.dateFrom && new Date(filters.dateFrom).toLocaleDateString('ar-SA')}
                            {' — '}
                    ...
                    <div style={{
                        ...
                        <div>
                            ...
                            <input type="date" value={filters.dateFrom}
                                onChange={e => { setFilters(p => ({ ...p, dateFrom: e.target.value })); setPage(1); setSelectedPeriod(''); }}
                                style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0' }} />
                        </div>
                        <div>
                            <label style={{ display: 'block', marginBottom: '6px', color: '#475569', fontSize: '13px', fontWeight: '500' }}>إلى تاريخ</label>
                            <input type="date" value={filters.dateTo}
                                onChange={e => { setFilters(p => ({ ...p, dateTo: e.target.value })); setPage(1); setSelectedPeriod(''); }}
                                style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0' }} />
                        </div>
                        <div>
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                                ...
                                <button onClick={() => setFilters(p => ({ ...p, sortOrder: p.sortOrder === 'asc' ? 'desc' : 'asc' }))}
                                    style={{ background: '#e0e7ff', border: 'none', padding: '10px', borderRadius: '8px', cursor: 'pointer' }}>
                                    <ArrowUpDown size={16} />
                                </button>
                            </div>
                        </div>
            ...
            <div style={{
                ...
                {loading ? (
                    <div style={{ padding: '60px', textAlign: 'center', color: '#64748b' }}>
                        <RefreshCw size={32} style={{ marginBottom: '12px', animation: 'spin 1s linear infinite' }} />
                        <p>جاري التحميل...</p>
                    </div>
                ) : expenses.length === 0 ? (
                    <div style={{ padding: '60px', textAlign: 'center', color: '#64748b' }}>
                        ...
                        <tbody>
                            {expenses.map(exp => (
                                <tr key={exp.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                                    ...
                                    <td style={{ padding: '16px' }}><CategoryChip category={exp.category} /></td>
                                    <td style={{ padding: '16px', fontWeight: '700', color: '#dc2626' }}>{fmt(exp.amount)} ج.م</td>
                                    <td style={{ padding: '16px', color: '#475569', maxWidth: '200px' }}>{exp.description || '-'}</td>
                                    <td style={{ padding: '16px', color: '#64748b' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            <Calendar size={14} />{formatDate(exp.expenseDate)}
                                        </div>
                                    </td>
                                    <td style={{ padding: '16px' }}><PaymentBadge method={exp.paymentMethod} /></td>
                                    <td style={{ padding: '16px', color: '#64748b' }}>{exp.user.fullName}</td>
                                    <td style={{ padding: '16px' }}>
                ...
                {totalPages > 1 && (
                    <div style={{
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            ...
                            <span style={{ padding: '8px 16px', color: '#475569', fontWeight: '600' }}>{page} / {totalPages}</span>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                            <div style={{ gridColumn: 'span 2' }}>
                                ...
                                <select value={expenseForm.categoryId}
                                    ...
                                    style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '2px solid #e2e8f0', fontSize: '14px' }}>
                                    <option value="">اختر التصنيف</option>
                                    {activeCategories.map(c => <option key={c.id} value={c.id}>{c.nameAr || c.name}</option>)}
                                </select>
                            </div>
                            <div>
                            ...
                            <div style={{ gridColumn: 'span 2' }}>
                                ...
                                <textarea value={expenseForm.description}
                                    ...
                                    placeholder="وصف المصروف..." rows={2}
                                    style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '2px solid #e2e8f0', fontSize: '14px', resize: 'none' }} />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="text" value={expenseForm.reference}
                            ...
                            <div style={{ gridColumn: 'span 2', display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                {expenseForm.isRecurring && (
                                    <input type="number" min="1" max="31" value={expenseForm.recurringDay}
                                        onChange={e => setExpenseForm(p => ({ ...p, recurringDay: e.target.value }))}
                                        placeholder="يوم الشهر"
                                        style={{ width: '100px', padding: '8px', borderRadius: '8px', border: '2px solid #e2e8f0', fontSize: '14px' }} />
                                )}
                            </div>
                        </div>
            ...
            {categoryModal && (
                <div style={{
                    ...
                    <div style={{ background: 'white', borderRadius: '20px', padding: '32px', width: '95%', maxWidth: '450px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                            <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1e293b', margin: 0 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            ...
                            <div>
                                ...
                                <input type="text" value={categoryForm.nameAr}
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                    {['#6366f1', '#3b82f6', '#14b8a6', '#16a34a', '#d97706', '#dc2626', '#ec4899', '#8b5cf6'].map(color => (
                                        <button key={color} onClick={() => setCategoryForm(p => ({ ...p, color }))} style={{
                                            width: '36px', height: '36px', borderRadius: '8px', background: color,
                                            border: categoryForm.color === color ? '3px solid #1e293b' : '2px solid #e2e8f0', cursor: 'pointer'
                                        }} />
                                    ))}
                                </div>
                            </div>
            ...
            {deleteModal && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#1e293b', marginBottom: '8px' }}>تأكيد الحذف</h3>
                        <p style={{ color: '#64748b', marginBottom: '24px' }}>
                            {deleteModal.type === 'expense' ? 'هل أنت متأكد من حذف هذا المصروف؟' : 'هل أنت متأكد من حذف هذا التصنيف؟'}
                        </p>
                        <div style={{ display: 'flex', gap: '12px' }}>
