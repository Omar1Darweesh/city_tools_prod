...
export default function EditUserPage() {
  ...
  const [form, setForm] = useState({ username: "", fullName: "", password: "", roleId: 1, active: true });

  const set = (key: string, value: any) => setForm((f) => ({ ...f, [key]: value }));

  useEffect(() => {
    adminApi.getUser(Number(params.id)).then((res: any) => {
      const u = res.data;
      if (!u) { router.push("/admin/users"); return; }
      const roleId = u.roles?.[0]?.roleId || u.roles?.[0]?.role?.id || 1;
      setForm({ username: u.username, fullName: u.fullName, password: "", roleId, active: u.active ?? true });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const payload: any = { username: form.username, fullName: form.fullName, roleId: form.roleId, active: form.active };
      if (form.password.trim()) payload.password = form.password;
      await adminApi.updateUser(Number(params.id), payload);
    ...
    <div className="cf-root">
      <div className="cf-header">
        ...
        <div className="cf-title-row">
          ...
          <div>
            <h1 className="cf-title">{isRtl ? "تعديل المستخدم" : "Edit User"}</h1>
            <p className="cf-sub">{form.fullName || form.username}</p>
          </div>
        </div>
      </div>
        ...
        <div className="cf-section">
          ...
          <div className="cf-grid-2">
            <div className="cf-field">
              ...
              <input className="cf-input" value={form.username} onChange={(e) => set("username", e.target.value)} required />
            </div>
            <div className="cf-field">
            ...
            <div className="cf-field cf-field-full">
              <label className="cf-label">{isRtl ? "كلمة المرور الجديدة" : "New Password"}</label>
              <input className="cf-input" type="password" value={form.password} onChange={(e) => set("password", e.target.value)} autoComplete="new-password" />
              <p className="cf-field-hint">{isRtl ? "اتركه فارغاً إذا لم ترد تغيير كلمة المرور" : "Leave blank to keep current password"}</p>
            </div>
            <div className="cf-field cf-field-full">
