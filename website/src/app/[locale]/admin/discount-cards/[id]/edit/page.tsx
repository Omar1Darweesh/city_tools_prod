...
import { ArrowLeft, Save, Percent, Loader2 } from "lucide-react";
...
const DISCOUNT_CARD_IMAGES = [
  "/assets/discountCards/11.jpg",
  "/assets/discountCards/22.jpg",
];

const bgColorOptions = [
  { value: "linear-gradient(135deg, #0f1923 0%, #1a2535 40%, #C0161B 100%)", labelEn: "Dark with Red Accent", labelAr: "داكن مع ذهبي", demo: "#0f1923" },
...
export default function EditDiscountCardPage() {
  ...
  const [form, setForm] = useState({
    ...
    titleEn: "", titleAr: "",
    descEn: "", descAr: "",
    linkUrl: "", linkLabelEn: "", linkLabelAr: "",
    bgColor: "", bgImage: DISCOUNT_CARD_IMAGES[0], isActive: true,
  });

  const set = (key: string, value: any) => setForm(f => ({ ...f, [key]: value }));

  useEffect(() => {
    adminApi.getDiscountCard(Number(params.id)).then((res) => {
      const c = res.data;
      setForm({
        badgeEn: c.badgeEn, badgeAr: c.badgeAr,
        titleEn: c.titleEn, titleAr: c.titleAr,
        descEn: c.descEn, descAr: c.descAr,
        linkUrl: c.linkUrl, linkLabelEn: c.linkLabelEn, linkLabelAr: c.linkLabelAr,
        bgColor: c.bgColor, bgImage: c.bgImage || DISCOUNT_CARD_IMAGES[0], isActive: c.isActive,
      });
    }).catch(() => router.push("/admin/discount-cards")).finally(() => setLoading(false));
  }, [params.id, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminApi.updateDiscountCard(Number(params.id), form);
    ...
    <div className="cf-root">
      <div className="cf-header">
        ...
        <div className="cf-title-row">
          ...
          <div>
            <h1 className="cf-title">{isRtl ? "تعديل بطاقة الخصم" : "Edit Discount Card"}</h1>
            <p className="cf-sub">{isRtl ? form.titleAr : form.titleEn}</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="cf-form">
        <div className="cf-section">
          ...
          <div className="cf-grid-2">
            <div className="cf-field">
              ...
              <input className="cf-input" value={form.badgeEn} onChange={e => set("badgeEn", e.target.value)} />
            </div>
            <div className="cf-field">
              <label className="cf-label">{isRtl ? "شارة (عربي)" : "Badge (Arabic)"}</label>
              <input className="cf-input" value={form.badgeAr} onChange={e => set("badgeAr", e.target.value)} dir="rtl" />
            </div>
            <div className="cf-field">
            ...
            <div className="cf-field">
              ...
              <input className="cf-input" value={form.titleAr} onChange={e => set("titleAr", e.target.value)} required dir="rtl" />
            </div>
            <div className="cf-field cf-field-full">
              <label className="cf-label">{isRtl ? "الوصف (إنجليزي)" : "Description (English)"} <span className="cf-req">*</span></label>
              <textarea className="cf-input cf-textarea" value={form.descEn} onChange={e => set("descEn", e.target.value)} required rows={2} />
            </div>
            <div className="cf-field cf-field-full">
              <label className="cf-label">{isRtl ? "الوصف (عربي)" : "Description (Arabic)"} <span className="cf-req">*</span></label>
              <textarea className="cf-input cf-textarea" value={form.descAr} onChange={e => set("descAr", e.target.value)} required rows={2} dir="rtl" />
            </div>
          </div>
        </div>
        ...
        <div className="cf-section">
          ...
          <div className="cf-grid-2">
            ...
            <div className="cf-field">
              ...
              <input className="cf-input" value={form.linkLabelEn} onChange={e => set("linkLabelEn", e.target.value)} />
            </div>
            <div className="cf-field">
              <label className="cf-label">{isRtl ? "نص الزر (عربي)" : "Button Label (Arabic)"}</label>
              <input className="cf-input" value={form.linkLabelAr} onChange={e => set("linkLabelAr", e.target.value)} dir="rtl" />
            </div>
          </div>
        </div>
        ...
        <div className="cf-section">
          ...
          <div className="cf-grid-2">
            ...
            <div className="cf-field cf-field-full">
              <label className="cf-label">{isRtl ? "صورة الخلفية" : "Background Image"}</label>
              <div className="flex flex-wrap gap-3">
                {DISCOUNT_CARD_IMAGES.map((img) => (
                  <button
                    key={img}
                    type="button"
                    onClick={() => set("bgImage", img)}
            ...
            <div className="cf-field cf-field-full">
              <label className="cf-preview-box" style={{ background: form.bgColor || "transparent", minHeight: "60px", borderRadius: "0.75rem", padding: "0.75rem 1rem" }}>
                <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "#e83030", background: "rgba(192,22,27,0.2)", border: "1px solid rgba(192,22,27,0.3)", borderRadius: "999px", padding: "0.15rem 0.5rem", width: "fit-content", marginBottom: "0.35rem" }}>✦ {form.badgeEn}</span>
                <span style={{ fontSize: "0.9rem", fontWeight: 800, color: "#fff" }}>{form.titleEn || "Title"}</span>
              </label>
            </div>
            <div className="cf-field cf-field-full">
