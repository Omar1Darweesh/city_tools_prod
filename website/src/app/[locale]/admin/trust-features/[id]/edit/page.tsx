...
export default function EditTrustFeaturePage() {
  ...
  useEffect(() => {
    adminApi.getTrustFeature(id).then((res: any) => {
      const d = res.data;
      if (!d) {
        setError(isRtl ? "غير موجود" : "Not found");
        return;
      }
      setForm({
        icon: d.icon || "Shield",
    ...
    }).catch(() => setError(isRtl ? "فشل التحميل" : "Failed to load")).finally(() => setLoading(false));
  }, [id, isRtl]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminApi.updateTrustFeature(id, form);
  ...
  if (loading) return <div className="tf-loading"><Loader2 className="size-6 animate-spin" /></div>;

  if (error && !form.titleEn) return (
    <div className="cf-root">
      <div className="cf-error">{error}</div>
      <Link href="/admin/trust-features" className="cf-back"><ArrowLeft className="size-4" />{isRtl ? "العودة" : "Back"}</Link>
    </div>
  );
    ...
    <div className="cf-root">
      <div className="cf-header">
        ...
        <div className="cf-title-row">
          ...
          <div>
            <h1 className="cf-title">{isRtl ? "تعديل الميزة" : "Edit Trust Feature"}</h1>
            <p className="cf-sub">{isRtl ? "تعديل بيانات ميزة الثقة" : "Update trust feature details"}</p>
          </div>
        </div>
      </div>
