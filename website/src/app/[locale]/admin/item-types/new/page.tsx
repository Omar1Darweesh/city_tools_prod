...
export default function NewItemTypePage() {
  ...
  useEffect(() => {
    adminApi.getSubcategories()
      .then(res => {
        ...
        if (scs.length > 0) setSubcategoryId(String(scs[0].id));
      })
      .catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await adminApi.createItemType({ name: name.trim(), nameAr: nameAr.trim(), subcategoryId: Number(subcategoryId) });
    ...
    <div className="aitf-root">
      <div className="aitf-header">
        ...
        <div className="aitf-title-row">
          <div className="aitf-title-icon">
            <List className="size-5 text-white" />
          </div>
          <div>
            <h1 className="aitf-title">{isRtl ? "إضافة نوع" : "Add Item Type"}</h1>
            <p className="aitf-sub">{isRtl ? "أضف نوعاً جديداً" : "Add a new item type"}</p>
          </div>
        </div>
      </div>
