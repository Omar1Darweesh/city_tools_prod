...
export default function NewStatisticPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminApi.createStatistic(form);
    ...
    <div className="cf-root">
      <div className="cf-header">
        ...
        <div className="cf-title-row">
          ...
          <div>
            <h1 className="cf-title">{isRtl ? "إضافة إحصاء جديد" : "Add New Statistic"}</h1>
            <p className="cf-sub">{isRtl ? "أضف إحصاءً جديداً للصفحة الرئيسية" : "Add a new counter for the homepage"}</p>
          </div>
        </div>
      </div>
        ...
        <div className="cf-section">
          ...
          <div className="cf-grid-2">
            <div className="cf-field">
              ...
              <p className="cf-field-hint">{isRtl ? "مثال: 500+, 1000+, 11+" : "e.g. 500+, 1000+, 11+"}</p>
            </div>
            <div className="cf-field">
            ...
            <div className="cf-field">
              ...
              <input className="cf-input" value={form.labelEn} onChange={e => set("labelEn", e.target.value)} required placeholder="e.g. Products" />
            </div>
            <div className="cf-field">
              <label className="cf-label">{isRtl ? "العنوان بالعربية" : "Label (Arabic)"} <span className="cf-req">*</span></label>
              <input className="cf-input" value={form.labelAr} onChange={e => set("labelAr", e.target.value)} required dir="rtl" placeholder="مثال: منتج متاح" />
            </div>
        ...
        <div className="cf-actions">
          <button type="submit" disabled={saving} className="cf-save-btn" style={{ background: "#8b5cf6" }}>
            {saving ? (
              ...
              <><Save className="size-4" />{isRtl ? "حفظ الإحصاء" : "Save Statistic"}</>
            )}
          </button>
