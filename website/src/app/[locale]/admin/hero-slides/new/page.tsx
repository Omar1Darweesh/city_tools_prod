    ...
    <div className="cf-root">
      <div className="cf-header">
        ...
        <div className="cf-title-row">
          ...
          <div>
            <h1 className="cf-title">{isRtl ? "إضافة شريحة جديدة" : "Add New Hero Slide"}</h1>
            <p className="cf-sub">{isRtl ? "أضف شريحة جديدة للقسم العلوي في الصفحة الرئيسية" : "Add a new slide for the homepage hero section"}</p>
          </div>
        </div>
      </div>
        ...
        <div className="cf-section">
          ...
          <div className="cf-grid-2">
            ...
            <div className="cf-field">
              ...
              <p className="cf-field-hint">{isRtl ? "اختر نمط التدرج للخلفية" : "Choose a gradient style for the overlay"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="cf-section">
          ...
          <div className="cf-grid-2">
            <div className="cf-field">
              ...
              <input className="cf-input" value={form.tagEn} onChange={e => set("tagEn", e.target.value)} placeholder="500+ Products" />
            </div>
            <div className="cf-field">
              <label className="cf-label">{isRtl ? "الوسم (عربي)" : "Tag (Arabic)"}</label>
              <input className="cf-input" value={form.tagAr} onChange={e => set("tagAr", e.target.value)} dir="rtl" placeholder="500+ منتج" />
            </div>
            <div className="cf-field">
              <label className="cf-label">{isRtl ? "العنوان (إنجليزي)" : "Title (English)"}</label>
              <input className="cf-input" value={form.titleEn} onChange={e => set("titleEn", e.target.value)} placeholder="Professional Tools" />
            </div>
            <div className="cf-field">
              <label className="cf-label">{isRtl ? "العنوان (عربي)" : "Title (Arabic)"}</label>
              <input className="cf-input" value={form.titleAr} onChange={e => set("titleAr", e.target.value)} dir="rtl" placeholder="أدوات المحترفين" />
            </div>
            <div className="cf-field">
              <label className="cf-label">{isRtl ? "الوصف (إنجليزي)" : "Subtitle (English)"}</label>
              <input className="cf-input" value={form.subEn} onChange={e => set("subEn", e.target.value)} placeholder="Unmatched quality · Competitive prices" />
            </div>
            <div className="cf-field">
              <label className="cf-label">{isRtl ? "الوصف (عربي)" : "Subtitle (Arabic)"}</label>
              <input className="cf-input" value={form.subAr} onChange={e => set("subAr", e.target.value)} dir="rtl" placeholder="جودة لا تُضاهى · أسعار تنافسية" />
            </div>
            <div className="cf-field">
        ...
        <div className="cf-actions">
          <button type="submit" disabled={saving} className="cf-save-btn" style={{ background: "#7c3aed" }}>
            {saving ? (
              ...
              <><Save className="size-4" />{isRtl ? "حفظ الشريحة" : "Save Slide"}</>
            )}
          </button>
