...
export default function EditSubcategoryPage() {
  ...
  useEffect(() => {
    ...
    adminApi.getSubcategories()
      .then((res) => {
        ...
        if (!item) {
          setError(isRtl ? "لم يتم العثور على التصنيف الفرعي" : "Subcategory not found");
          return;
        }
        setName(item.name || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await adminApi.updateSubcategory(Number(params.id), { name: name.trim(), nameAr: nameAr.trim(), categoryId: Number(categoryId) });
  ...
  if (loading) {
    return (
      <div className="ascf-root">
        <div className="ascf-loading">{isRtl ? "جاري التحميل..." : "Loading..."}</div>
    ...
    <div className="ascf-root">
      <div className="ascf-header">
        ...
        <div className="ascf-title-row">
          <div className="ascf-title-icon" style={{ background: "linear-gradient(135deg, #3b82f6, #1d4ed8)" }}>
            <Layers className="size-5 text-white" />
          </div>
          <div>
            <h1 className="ascf-title">{isRtl ? "تعديل التصنيف الفرعي" : "Edit Subcategory"}</h1>
            <p className="ascf-sub">{name || `#${params.id}`}</p>
          </div>
        </div>
      </div>
      ...
      <style>{ascfStyles}</style>
      <style>{`.ascf-loading { padding: 2rem; color: var(--muted-foreground); }`}</style>
    </div>
  );
}
